...
export default async function OwnerDashboardPage() {
  ...
  const statusBadge = (status: string): "warning" | "success" | "destructive" | "muted" => {
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-600">
                        {biz.category?.name ?? "Categorie"} - {biz._count?.reviews ?? 0} avis
                      </p>
                    </div>
                    <Badge variant={statusBadge(biz.status)}>{statusLabel(biz.status)}</Badge>
                    <Link
                  ...
                  {biz.status === "REJECTED" && biz.rejectReason ? (
                    <div className="mt-3 rounded-lg border border-rose-100 bg-rose-50 px-3 py-2 text-xs text-rose-700">
                      <p className="font-semibold text-rose-800">Motif du rejet</p>
                      <p className="mt-1 whitespace-pre-wrap">{biz.rejectReason}</p>
                    </div>
                  ) : null}
                </Card>
              ))}
